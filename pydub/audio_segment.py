...
def extract_wav_headers(data):
    ...
    while pos + 8 < len(data) and len(subchunks) < 10:
        ...
        subchunk_size = struct.unpack_from('<I', memoryview(data[pos + 4:pos + 8]))[0]
...
def read_wav_audio(data, headers=None):
    ...
    fmt = fmt[0]
    pos = fmt.position + 8
    audio_format = struct.unpack_from('<H', memoryview(data[pos:pos + 2]))[0]
    if audio_format != 1 and audio_format != 0xFFFE:
        raise CouldntDecodeError("Unknown audio format 0x%X in wav data" %
                                 audio_format)

    channels = struct.unpack_from('<H', memoryview(data[pos + 2:pos + 4]))[0]
    sample_rate = struct.unpack_from('<I', memoryview(data[pos + 4:pos + 8]))[0]
    bits_per_sample = struct.unpack_from('<H', memoryview(data[pos + 14:pos + 16]))[0]
